...
public interface BannerRepository extends JpaRepository<Banner, Long> {
    ...
    @Query("SELECT b FROM Banner b WHERE b.deleted=false AND category=:cat AND b NOT IN " +
            "(SELECT r.banner FROM Request r WHERE r.userAgent=:#{#rq.userAgent} AND r.ipAddress=:#{#rq.ipAddress} AND r.date >= CURRENT_DATE)")
    List<Banner> getActiveBannersByCategoryWithSorting(@Param("cat") Category category, @Param("rq") Request request, Pageable pageable);
